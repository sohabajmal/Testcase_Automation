...
def listen_tcpdump(compute_ip):
    ...
    command= "sudo timeout 30 tcpdump -vv -nnn -i {} | grep 'ICMP echo'".format(port_to_listen)
    try:
        ...
        logging.info("command {} successfully executed on compute node {}".format(command, compute_ip))
...
def ping_instances(instance1, instance2, settings, command):
    try:
        ...
        client.connect(instance1, port=22, username="centos", key_filename=os.path.expanduser(settings["key_file"]))
        logging.info("SSH Session is established")
        logging.info("Running command in a compute node")
        ...
        logging.exception(e)
        logging.error("error ocurred when making ssh connection and running command on remote instance") 
    finally:
        client.close()
        logging.info("Connection from client has been closed") 
...
def verify_offloading_on_representor_port(compute_ip, ip, ping_ip, settings):
    ...
    instance_ping_thread= Thread(target=ping_instances, args=(ip, ping_ip, settings, command))
...
def get_sriov_enabled_interfaces(baremetal_nodes):
    #get sriov ports
    result= os.popen("cat ~/pilot/templates/neutron-sriov.yaml |grep physint:").read()
    result= result.split('\n')
    interfaces=result[:-1]
    ...
    try:
        for node in baremetal_nodes:
            for interface in interfaces:
                ...
                result= ssh_into_node(node, command)
