...
def verify_dvr_agent_on_nodes(baremetal_nodes, node_type):
    ...
    try:
        for node in baremetal_nodes:
            ...
            logging.debug("agent mode of node {}, is {}".format(node, ssh_output[0].strip()))
...
def listen_tcpdump(host_ip, namespace, namespace_type):
    try:
        ...
        ssh_session = ssh_client.connect(host_ip, username="heat-admin",key_filename=os.path.expanduser("~/.ssh/id_rsa"))
...
def get_namespace_id(node_ip, namespace_type):
    ...
    namespace= ssh_into_node(node_ip, command)
